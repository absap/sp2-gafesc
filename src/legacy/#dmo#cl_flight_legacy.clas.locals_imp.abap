CLASS lcl_booking_supplement_buffer DEFINITION FINAL CREATE PRIVATE.
  PUBLIC SECTION.
    ...
    " If we delete a travel we have to delete (1) booking supplements (2) bookings (3) the travel in this order!
    " But we may also want to delete a specific booking only.
    " As an initial booking ID is not a valid value we can use this to distinguish the two cases.
    METHODS delete_booking_supplements IMPORTING it_booking TYPE /dmo/if_flight_legacy=>tt_booking.
    METHODS save.
    METHODS initialize.
  ...
  PRIVATE SECTION.
    ...
    METHODS _check
      IMPORTING
        is_booking_supplement  TYPE /dmo/book_suppl
      ...
      CHANGING
        ct_messages            TYPE /dmo/if_flight_legacy=>tt_if_t100_message
      RETURNING
        VALUE(rv_is_valid)     TYPE abap_bool.
    METHODS _check_supplement
      IMPORTING
        is_booking_supplement  TYPE /dmo/book_suppl
        is_booking_supplementx TYPE /dmo/if_flight_legacy=>ts_booking_supplementx OPTIONAL
        iv_change_mode         TYPE /dmo/cl_flight_legacy=>ty_change_mode
...
CLASS lcl_booking_supplement_buffer IMPLEMENTATION.
  ...
  METHOD _update.
    ...
    LOOP AT it_booking_supplement ASSIGNING <s_booking_supplement_update>.
      ...
      READ TABLE mt_create_buffer INTO ls_buffer_booking_supplement WITH TABLE KEY travel_id             = <s_booking_supplement_update>-travel_id
                                                                                   booking_id            = <s_booking_supplement_update>-booking_id
                                                                                   booking_supplement_id = <s_booking_supplement_update>-booking_supplement_id." Special case: record already in create buffer
      IF sy-subrc = 0.
      ...
      " Copy back all non-key fields
      <s_buffer_booking_supplement>-supplement_id = ls_booking_supplement-supplement_id.
      <s_buffer_booking_supplement>-price         = ls_booking_supplement-price.
      <s_buffer_booking_supplement>-currency_code = ls_booking_supplement-currency_code.
  ...
  METHOD delete_booking_supplements.
    " if it_booking_key[...]-booking_id is initial: delete all booking supplements of it_booking_key[...]-travel_id

    CHECK it_booking IS NOT INITIAL.

    " For any given Travel ID, Booking ID ...

    LOOP AT it_booking ASSIGNING FIELD-SYMBOL(<s_booking_key>).
      " ... remove all entries from the create buffer
      IF <s_booking_key>-booking_id IS INITIAL.
        DELETE mt_create_buffer WHERE travel_id = <s_booking_key>-travel_id.
      ELSE.
        DELETE mt_create_buffer WHERE travel_id = <s_booking_key>-travel_id AND booking_id = <s_booking_key>-booking_id.
      ENDIF.

      " ... move all entries from the update buffer to the delete buffer
      LOOP AT mt_update_buffer ASSIGNING FIELD-SYMBOL(<s_update_buffer>) WHERE travel_id = <s_booking_key>-travel_id.
        IF <s_booking_key>-booking_id IS NOT INITIAL AND <s_booking_key>-booking_id <> <s_update_buffer>-booking_id.
          CONTINUE.
        ENDIF.
        INSERT VALUE #( travel_id = <s_update_buffer>-travel_id  booking_id = <s_update_buffer>-booking_id ) INTO TABLE mt_delete_buffer.
        DELETE mt_update_buffer.
      ENDLOOP.
    ENDLOOP.

    " ... add all remaining entries from the database to the delete buffer
    DATA lt_booking_supplement_key TYPE /dmo/if_flight_legacy=>tt_booking_supplement_key.
    SELECT travel_id, booking_id, booking_supplement_id FROM /dmo/book_suppl FOR ALL ENTRIES IN @it_booking WHERE travel_id = @it_booking-travel_id INTO CORRESPONDING FIELDS OF TABLE @lt_booking_supplement_key.
    LOOP AT lt_booking_supplement_key ASSIGNING FIELD-SYMBOL(<s_booking_supplement_key>).
      READ TABLE it_booking TRANSPORTING NO FIELDS WITH TABLE KEY travel_id = <s_booking_supplement_key>-travel_id  booking_id = <s_booking_supplement_key>-booking_id.
      IF sy-subrc <> 0.
        READ TABLE it_booking TRANSPORTING NO FIELDS WITH TABLE KEY travel_id = <s_booking_supplement_key>-travel_id  booking_id = '0'.
      ENDIF.
      IF sy-subrc = 0.
        READ TABLE mt_delete_buffer TRANSPORTING NO FIELDS WITH TABLE KEY travel_id             = <s_booking_supplement_key>-travel_id
                                                                          booking_id            = <s_booking_supplement_key>-booking_id
                                                                          booking_supplement_id = <s_booking_supplement_key>-booking_supplement_id.
        IF sy-subrc <> 0.
          INSERT VALUE #( travel_id             = <s_booking_supplement_key>-travel_id
                          booking_id            = <s_booking_supplement_key>-booking_id
                          booking_supplement_id = <s_booking_supplement_key>-booking_supplement_id ) INTO TABLE mt_delete_buffer.
        ENDIF.
      ENDIF.
    ENDLOOP.
  ENDMETHOD.
  ...
  METHOD get.
    ...
    LOOP AT it_booking_supplement ASSIGNING FIELD-SYMBOL(<s_booking_supplement>).
      ...
      LOOP AT mt_update_buffer ASSIGNING FIELD-SYMBOL(<s_update_buffer>) WHERE travel_id = <s_booking_supplement>-travel_id AND booking_id = <s_booking_supplement>-booking_id.
        INSERT <s_update_buffer> INTO TABLE et_booking_supplement.
      ENDLOOP.
    ENDLOOP.

    SELECT * FROM /dmo/book_suppl FOR ALL ENTRIES IN @it_booking_supplement WHERE travel_id             = @it_booking_supplement-travel_id
                                                                              AND booking_id            = @it_booking_supplement-booking_id
                                                              INTO TABLE @DATA(lt_booking_supplement_db) ##SELECT_FAE_WITH_LOB[DESCRIPTION].

    INSERT LINES OF lt_booking_supplement_db INTO TABLE et_booking_supplement.
...
CLASS lcl_booking_buffer DEFINITION FINAL CREATE PRIVATE.
  PUBLIC SECTION.
    ...
    METHODS delete_bookings IMPORTING it_travel_key TYPE /dmo/if_flight_legacy=>tt_travel_key.
    METHODS save.
    METHODS initialize.
  ...
  PRIVATE SECTION.
    ...
    DATA mt_customer_id TYPE SORTED TABLE OF /dmo/customer_id WITH UNIQUE KEY table_line.
...
CLASS lcl_booking_buffer IMPLEMENTATION.
  ...
  METHOD _update.
    ...
    LOOP AT it_booking ASSIGNING <s_booking_update>.
      ...
      READ TABLE mt_create_buffer INTO ls_buffer_booking WITH TABLE KEY travel_id = <s_booking_update>-travel_id  booking_id = <s_booking_update>-booking_id." Special case: record already in create buffer
      IF sy-subrc = 0.
  ...
  METHOD delete_bookings.
    CHECK it_travel_key IS NOT INITIAL.

    " For any given Travel ID ...

    LOOP AT it_travel_key ASSIGNING FIELD-SYMBOL(<s_travel_key>).
      " ... remove all entries from the create buffer
      DELETE mt_create_buffer WHERE travel_id = <s_travel_key>-travel_id.

      " ... move all entries from the update buffer to the delete buffer
      LOOP AT mt_update_buffer ASSIGNING FIELD-SYMBOL(<s_update_buffer>) WHERE travel_id = <s_travel_key>-travel_id.
        INSERT VALUE #( travel_id = <s_update_buffer>-travel_id  booking_id = <s_update_buffer>-booking_id ) INTO TABLE mt_delete_buffer.
        DELETE mt_update_buffer.
      ENDLOOP.
    ENDLOOP.
    ...
    " ... add all remaining entries from the database to the delete buffer
    DATA lt_booking_key TYPE /dmo/if_flight_legacy=>tt_booking_key.
    SELECT travel_id, booking_id FROM /dmo/booking FOR ALL ENTRIES IN @it_travel_key WHERE travel_id = @it_travel_key-travel_id INTO CORRESPONDING FIELDS OF TABLE @lt_booking_key.
    LOOP AT lt_booking_key ASSIGNING FIELD-SYMBOL(<s_booking_key>).
      READ TABLE mt_delete_buffer TRANSPORTING NO FIELDS WITH TABLE KEY travel_id = <s_booking_key>-travel_id  booking_id = <s_booking_key>-booking_id.
      IF sy-subrc <> 0.
        INSERT VALUE #( travel_id = <s_booking_key>-travel_id  booking_id = <s_booking_key>-booking_id ) INTO TABLE mt_delete_buffer.
      ENDIF.
    ENDLOOP.
  ENDMETHOD.
  ...
  METHOD get.
    ...
    LOOP AT it_booking ASSIGNING FIELD-SYMBOL(<s_booking>).
      ...
      LOOP AT mt_update_buffer ASSIGNING FIELD-SYMBOL(<s_update_buffer>) WHERE travel_id = <s_booking>-travel_id.
        INSERT <s_update_buffer> INTO TABLE et_booking.
      ENDLOOP.
    ENDLOOP.

    SELECT * FROM /dmo/booking FOR ALL ENTRIES IN @it_booking WHERE travel_id  = @it_booking-travel_id
                                                              INTO TABLE @DATA(lt_booking_db) ##SELECT_FAE_WITH_LOB[DESCRIPTION].

    INSERT LINES OF lt_booking_db INTO TABLE et_booking.
  ...
  METHOD _check_customer.
    ...
    IF iv_change_mode = /dmo/cl_flight_legacy=>change_mode-create OR ( iv_change_mode = /dmo/cl_flight_legacy=>change_mode-update AND is_bookingx-customer_id = abap_true ).
      ...
      ENDIF.
      READ TABLE mt_customer_id TRANSPORTING NO FIELDS WITH TABLE KEY table_line = is_booking-customer_id.
      IF sy-subrc <> 0.
        rv_is_valid = abap_false.
        APPEND NEW /dmo/cx_flight_legacy( textid = /dmo/cx_flight_legacy=>customer_unkown  customer_id = is_booking-customer_id ) TO ct_messages.
      ENDIF.
    ENDIF.
  ENDMETHOD.
...
CLASS lcl_travel_buffer DEFINITION FINAL CREATE PRIVATE.
  ...
  PRIVATE SECTION.
    ...
    METHODS _check_status IMPORTING is_travel          TYPE /dmo/travel
                          ...
                          CHANGING  ct_messages        TYPE /dmo/if_flight_legacy=>tt_if_t100_message
                          RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _update_admin CHANGING cs_travel_admin TYPE /dmo/travel_admin.
ENDCLASS.
...
CLASS lcl_travel_buffer IMPLEMENTATION.
  ...
  METHOD _update.
    ...
    LOOP AT it_travel ASSIGNING FIELD-SYMBOL(<s_travel_update>).
      ...
      READ TABLE mt_create_buffer INTO ls_buffer_travel WITH TABLE KEY travel_id = <s_travel_update>-travel_id." Special case: record already in create buffer
      IF sy-subrc = 0.
      ...
      _update_admin( CHANGING cs_travel_admin = <s_buffer_travel>-gr_admin ).
  ...
  METHOD set_status_to_booked.
    ...
    _update_admin( CHANGING cs_travel_admin = <s_travel>-gr_admin ).
  ENDMETHOD.
  ...
  METHOD get.
    ...
    LOOP AT lt_travel ASSIGNING FIELD-SYMBOL(<s_travel>).
      ...
      IF sy-subrc = 0.
        DELETE lt_travel.
        INSERT <s_update_buffer> INTO TABLE et_travel.
        CONTINUE.
      ENDIF.
    ...
    SELECT * FROM /dmo/travel FOR ALL ENTRIES IN @lt_travel WHERE travel_id = @lt_travel-travel_id INTO TABLE @DATA(lt_travel_db) ##SELECT_FAE_WITH_LOB[DESCRIPTION].

    INSERT LINES OF lt_travel_db INTO TABLE et_travel.
  ENDMETHOD.
  ...
  METHOD _check_customer.
    ...
    IF iv_change_mode = /dmo/cl_flight_legacy=>change_mode-create OR ( iv_change_mode = /dmo/cl_flight_legacy=>change_mode-update AND is_travelx-customer_id = abap_true ).
      IF mt_customer_id IS INITIAL.
        SELECT DISTINCT customer_id FROM /dmo/customer INTO TABLE @mt_customer_id.
      ENDIF.
      READ TABLE mt_customer_id TRANSPORTING NO FIELDS WITH TABLE KEY table_line = is_travel-customer_id.
      IF sy-subrc <> 0.
        rv_is_valid = abap_false.
        APPEND NEW /dmo/cx_flight_legacy( textid = /dmo/cx_flight_legacy=>customer_unkown  customer_id = is_travel-customer_id ) TO ct_messages.
      ENDIF.
    ENDIF.
  ENDMETHOD.
