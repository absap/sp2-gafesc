CLASS ltc_base DEFINITION FOR TESTING DURATION SHORT RISK LEVEL HARMLESS.
  PROTECTED SECTION.
...
*   CLASS-DATA mr_test_environment TYPE REF TO if_osql_test_environment.
    ...
    CLASS-DATA mr_cut                      TYPE REF TO /dmo/cl_flight_legacy.

    CLASS-DATA mv_agency_id_1              TYPE /dmo/agency_id.
    CLASS-DATA mv_agency_id_2              TYPE /dmo/agency_id.
    CLASS-DATA mv_agency_id_unknown        TYPE /dmo/agency_id.

    CLASS-DATA mv_customer_id_1            TYPE /dmo/customer_id.
    CLASS-DATA mv_customer_id_2            TYPE /dmo/customer_id.
    CLASS-DATA mv_customer_id_unknown      TYPE /dmo/customer_id.

    CLASS-DATA mv_travel_id                TYPE /dmo/travel_id.
    CLASS-DATA mv_booking_id               TYPE /dmo/booking_id.

    CLASS-DATA mv_travel_id_unknown        TYPE /dmo/travel_id.
    CLASS-DATA mv_booking_id_unknown       TYPE /dmo/booking_id.
    CLASS-DATA mv_booking_suppl_id_unknown TYPE /dmo/booking_supplement_id.

    CLASS-DATA mv_carrier_id               TYPE /dmo/carrier_id.
    CLASS-DATA mv_carrier_id_unknown       TYPE /dmo/carrier_id.
    CLASS-DATA mv_connection_id            TYPE /dmo/connection_id.
    CLASS-DATA mv_flight_date_1            TYPE /dmo/flight_date.
    CLASS-DATA mv_flight_date_2            TYPE /dmo/flight_date.

    CLASS-DATA mv_booking_date             TYPE /dmo/booking_date.

    CLASS-DATA mv_supplement_id_1            TYPE /dmo/supplement_id.
    CLASS-DATA mv_supplement_price_1         TYPE /dmo/supplement_price.
    CLASS-DATA mv_supplement_currency_code_1 TYPE /dmo/currency_code.
    CLASS-DATA mv_supplement_id_2            TYPE /dmo/supplement_id.
    CLASS-DATA mv_supplement_price_2         TYPE /dmo/supplement_price ##NEEDED.
    CLASS-DATA mv_supplement_currency_code_2 TYPE /dmo/currency_code.
    CLASS-DATA mv_supplement_id_unknown      TYPE /dmo/supplement_id.
    ...
    METHODS _delete_existing_booking_suppl IMPORTING iv_travel_id             TYPE /dmo/travel_id
                                                     iv_booking_id            TYPE /dmo/booking_id
                                                     iv_booking_supplement_id TYPE /dmo/booking_supplement_id.
...
CLASS /dmo/cl_flight_legacy DEFINITION LOCAL FRIENDS ltc_base.

CLASS ltc_base IMPLEMENTATION.
  METHOD class_setup.
    IF mc_use_sql_doubles = abap_true ##BOOL_OK.
*     mr_test_environment = cl_osql_test_environment=>create( i_dependency_list = VALUE #( ( '/DMO/TRAVEL' ) ( '/DMO/BOOKING' ) ( '/DMO/BOOK_SUPPL' ) ( '/DMO/AGENCY' ) ( '/DMO/CUSTOMER' ) ) ).
    ELSE.
      ...
      ENDIF.
      mv_agency_id_1 = lt_agency_id[ 1 ].
      mv_agency_id_2 = lt_agency_id[ 2 ].
      cl_abap_unit_assert=>assert_differs( act = mv_agency_id_1  exp = mv_agency_id_2 )." To be totally sure

      " Select 2 known customer IDs
      IF lines( lt_customer_id ) < 2.
        cl_abap_unit_assert=>abort( msg = 'No customer data!' ).
      ENDIF.
      mv_customer_id_1 = lt_customer_id[ 1 ].
      mv_customer_id_2 = lt_customer_id[ 2 ].
      cl_abap_unit_assert=>assert_differs( act = mv_customer_id_1  exp = mv_customer_id_2 )." To be totally sure

      " Determine an unknown agency ID
      mv_agency_id_unknown = lt_agency_id[ 1 ].
      DO.
        mv_agency_id_unknown = mv_agency_id_unknown + 1.
        READ TABLE lt_agency_id   TRANSPORTING NO FIELDS WITH TABLE KEY table_line = mv_agency_id_unknown.
        IF sy-subrc <> 0.
          EXIT.
        ENDIF.
      ENDDO.
      ...
      mv_customer_id_unknown = lt_customer_id[ 1 ].
      DO.
        mv_customer_id_unknown = mv_customer_id_unknown + 1.
        READ TABLE lt_customer_id TRANSPORTING NO FIELDS WITH TABLE KEY table_line = mv_customer_id_unknown.
        IF sy-subrc <> 0.
          EXIT.
        ENDIF.
      ENDDO.
    ...
    " Select any valid combination of Travel ID, Booking ID
    SELECT SINGLE travel_id, MAX( booking_id ) FROM /dmo/booking GROUP BY travel_id INTO ( @mv_travel_id, @mv_booking_id ) ##WARN_OK.
    IF sy-subrc <> 0.
      cl_abap_unit_assert=>abort( 'No booking data!' ).
    ENDIF.

    " Determine an unknown Booking ID for the known Travel ID
    mv_booking_id_unknown = mv_booking_id + 1.
    ASSERT mv_booking_id_unknown IS NOT INITIAL.

    " Determine an unknown Booking Supplement ID for the known combination of Travel ID, Booking ID
    mv_booking_suppl_id_unknown = '0'.
    DO.
      mv_booking_suppl_id_unknown = mv_booking_suppl_id_unknown + 1.
      SELECT SINGLE booking_supplement_id FROM /dmo/book_suppl WHERE travel_id             = @mv_travel_id
                                                                 AND booking_id            = @mv_booking_id
                                                                 AND booking_supplement_id = @mv_booking_suppl_id_unknown
                                                                 INTO @DATA(lv_dummy_booking_supplement_id).
      IF sy-subrc <> 0.
        EXIT.
      ENDIF.
    ENDDO.

    " Select any valid combination of Carrier_ID, Connection_ID, and Flight_Date_1 / Flight_Date_2
    " Select a valid combination of Carrier_ID, Connection_ID with more than 1 Flight Date
    SELECT SINGLE carrier_id, connection_id, COUNT( * ) FROM /dmo/flight GROUP BY carrier_id, connection_id HAVING COUNT( * ) > 1 INTO ( @mv_carrier_id, @mv_connection_id, @DATA(lv_dummy_count) ) ##WARN_OK.
    IF sy-subrc <> 0.
      cl_abap_unit_assert=>abort( 'No useful flight data!' ).
    ENDIF.
    " Select 2 different Flight Dates
    SELECT SINGLE flight_date FROM /dmo/flight WHERE carrier_id = @mv_carrier_id AND connection_id = @mv_connection_id INTO @mv_flight_date_1 ##WARN_OK.
    SELECT SINGLE flight_date FROM /dmo/flight WHERE carrier_id = @mv_carrier_id AND connection_id = @mv_connection_id AND flight_date NE @mv_flight_date_1 INTO @mv_flight_date_2 ##WARN_OK.

    " Determine an unknown Carrier ID
    mv_carrier_id_unknown = 'XX'.
    SELECT SINGLE carrier_id FROM /dmo/carrier WHERE carrier_id = @mv_carrier_id_unknown INTO @DATA(lv_dummy_carrier_id).
    IF sy-subrc = 0.
      cl_abap_unit_assert=>abort( |Carrier ID { mv_carrier_id_unknown } should not be known!| ).
    ENDIF.

    " Determine an unknown travel ID
    mv_travel_id_unknown = '4999'.
    DO.
    ...
    SELECT SINGLE supplement_id, price, currency_code FROM /dmo/supplement INTO ( @mv_supplement_id_1, @mv_supplement_price_1, @mv_supplement_currency_code_1 ) ##WARN_OK.
    IF sy-subrc <> 0.
      cl_abap_unit_assert=>abort( 'No supplement data!' ).
    ENDIF.

    " Select a different valid combination of Supplement ID, Price, Currency Code
    SELECT SINGLE supplement_id, price, currency_code FROM /dmo/supplement WHERE supplement_id <> @mv_supplement_id_1 INTO ( @mv_supplement_id_2, @mv_supplement_price_2, @mv_supplement_currency_code_2 ) ##WARN_OK.
    IF sy-subrc <> 0.
      cl_abap_unit_assert=>abort( 'No supplement data!' ).
    ENDIF.

    " Determine an unknown Supplement ID
    mv_supplement_id_unknown = 'XX-999'.
    SELECT SINGLE supplement_id FROM /dmo/supplement WHERE supplement_id = @mv_supplement_id_unknown INTO @DATA(lv_dummy_supplement_id).
    IF sy-subrc = 0.
      cl_abap_unit_assert=>abort( |Supplement ID { mv_supplement_id_unknown } should not be known!| ).
    ENDIF.

    " Current date
    mv_booking_date = cl_abap_context_info=>get_system_date( ).
...
*     lt_agency = VALUE #( ( agency_id = mv_agency_id_1 ) ( agency_id = mv_agency_id_2 ) ).
*     mr_test_environment->insert_test_data( lt_agency ).
*
*     mv_customer_id_1 = '42'.
*     mv_customer_id_2 = '43'.
*     DATA lt_customer  TYPE STANDARD TABLE OF /dmo/customer.
*     lt_customer = VALUE #( ( customer_id = mv_customer_id_1 ) ( customer_id = mv_customer_id_2 ) ).
*     mr_test_environment->insert_test_data( lt_customer ).
*
*     mv_agency_id_unknown   = '99'.
*     mv_customer_id_unknown = '99'.
    ENDIF.
...
*
*     DATA(ls_travel_new) = VALUE /dmo/travel( travel_id     = lv_travel_id
*                                              agency_id     = is_travel_create-agency_id
*                                              customer_id   = is_travel_create-customer_id
*                                              description   = is_travel_create-description
*                                              begin_date    = is_travel_create-begin_date
*                                              end_date      = is_travel_create-end_date
*                                              status        = /dmo/if_flight_legacy=>travel_status-new
*                                              createdby     = sy-uname
*                                              lastchangedby = sy-uname
*                                              createdat     = lv_timestampl
*                                              lastchangedat = lv_timestampl ).
*     DATA lt_travel TYPE STANDARD TABLE OF /dmo/travel.
*     APPEND ls_travel_new TO lt_travel.
*     mr_test_environment->insert_test_data( lt_travel ).
*     MOVE-CORRESPONDING ls_travel_new TO rs_travel_created.
    ELSE.
      mr_cut->create_travel( EXPORTING is_travel   = is_travel
  ...
  METHOD _delete_existing_travel.
    mr_cut->delete_travel( EXPORTING iv_travel_id = iv_travel_id
  ...
  METHOD _create_booking.
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = iv_travel_id )
  ...
  METHOD _delete_existing_booking_suppl.
    mr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = iv_travel_id )
                                     is_travelx             = VALUE #( travel_id = iv_travel_id )
                                     it_booking_supplement  = VALUE #( ( booking_id            = iv_booking_id
                                                                         booking_supplement_id = iv_booking_supplement_id ) )
                                     it_booking_supplementx = VALUE #( ( booking_id            = iv_booking_id
                                                                         booking_supplement_id = iv_booking_supplement_id
...
CLASS ltc_travel DEFINITION FINAL FOR TESTING DURATION SHORT RISK LEVEL HARMLESS INHERITING FROM ltc_base.
  ...
  PRIVATE SECTION.
    ...
    "! Delete, delete a single travel in the same LUW -> NO Error
...
CLASS ltc_travel IMPLEMENTATION.
  METHOD create.
    ...
    mr_cut->create_travel( EXPORTING is_travel   = VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_2   begin_date = '20190101'  end_date = '20190201' )
  ...
  METHOD c_agency_unknown.
    ...
    mr_cut->create_travel( EXPORTING is_travel   = VALUE #( agency_id = mv_agency_id_unknown  customer_id = mv_customer_id_2 )
  ...
  METHOD c_customer_unknown.
    ...
    mr_cut->create_travel( EXPORTING is_travel   = VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_unknown )
                           IMPORTING es_travel   = DATA(ls_travel)
                                     et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( ls_travel-travel_id ).
  ...
  METHOD create_single.
    ...
    " Create a travel
    cl_abap_unit_assert=>assert_equals( act = _create_travel( VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_2  begin_date = '20190101'  end_date = '20190201'
                                                                       status = /dmo/if_flight_legacy=>travel_status-booked ) )-travel_id  exp = lv_travel_id_1 ).

    " Create a second travel
    cl_abap_unit_assert=>assert_equals( act = _create_travel( VALUE #( agency_id = mv_agency_id_2  customer_id = mv_customer_id_1  begin_date = '20190101'  end_date = '20190201'
    ...
    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-agency_id      exp = mv_agency_id_2 ).
    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-customer_id    exp = mv_customer_id_1 ).
  ...
  METHOD delete_single.
    ...
    DATA(ls_travel_new) = _create_travel( VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_2  begin_date = '20190101'  end_date = '20190201'  description = 'My_Description' ) ).
  ...
  METHOD update_single.
    ...
    DATA(ls_travel_new) = _create_travel( VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_2  begin_date = '20190101'  end_date = '20190201'  description = 'My_Text' ) ).
    ...
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_travel_new-travel_id  agency_id = mv_agency_id_unknown  description = 'My_New_Text' )
  ...
  METHOD d_travel_id_unknown.
    DATA(ls_travel_deleted) = _create_travel( VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_2  begin_date = '20190101'  end_date = '20190201' ) ).
    _delete_existing_travel( ls_travel_deleted-travel_id ).
    mr_cut->delete_travel( EXPORTING iv_travel_id = ls_travel_deleted-travel_id
                           IMPORTING et_messages  = DATA(lt_messages) ).
  ...
  METHOD u_travel_id_unknown.
    DATA(ls_travel_deleted) = _create_travel( VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_2  begin_date = '20190101'  end_date = '20190201'  description = 'My_Old_Text' ) ).
    _delete_existing_travel( ls_travel_deleted-travel_id ).
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_travel_deleted-travel_id  description = 'My_New_Text' )
  ...
  METHOD delete_multiple.
    ...
    DATA(ls_travel_new_1) = _create_travel( VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_2  begin_date = '20190101'  end_date = '20190201'  description = 'My_Text_1' ) ).
    cl_abap_unit_assert=>assert_equals( act = ls_travel_new_1-description  exp = 'My_Text_1' ).
    DATA(ls_travel_new_2) = _create_travel( VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_2  begin_date = '20190101'  end_date = '20190201'  description = 'My_Text_2' ) ).
    DATA(ls_travel_new_3) = _create_travel( VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_2  begin_date = '20190101'  end_date = '20190201'  description = 'My_Text_3' ) ).

    " Delete - only in buffer
    mr_cut->delete_travel( EXPORTING iv_travel_id = ls_travel_new_1-travel_id
                           IMPORTING et_messages  = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).

    mr_cut->delete_travel( EXPORTING iv_travel_id = ls_travel_new_2-travel_id
                           IMPORTING et_messages  = lt_messages ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).

    mr_cut->delete_travel( EXPORTING iv_travel_id = ls_travel_new_3-travel_id
                           IMPORTING et_messages  = lt_messages ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
  ...
  METHOD update_multiple.
    ...
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_travel_new_1-travel_id  agency_id = mv_agency_id_unknown  description = 'My_New_Text_1' )
                                     is_travelx  = VALUE #( travel_id = ls_travel_new_1-travel_id  description = abap_true )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).

    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_travel_new_2-travel_id  agency_id = mv_agency_id_unknown  description = 'My_New_Text_2' )
                                     is_travelx  = VALUE #( travel_id = ls_travel_new_2-travel_id  description = abap_true )
                           IMPORTING et_messages = lt_messages ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).

    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_travel_new_3-travel_id  agency_id = mv_agency_id_2        description = 'My_New_Text_3' )
  ...
  METHOD update_twice.
    ...
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_travel_new-travel_id  agency_id = mv_agency_id_2        description = 'WHATEVER' )
  ...
  METHOD update_delete_single.
    ...
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_travel_new-travel_id  agency_id = mv_agency_id_unknown  description = 'My_New_Text' )
                           ...
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
    mr_cut->delete_travel( EXPORTING iv_travel_id = ls_travel_new-travel_id
                           IMPORTING et_messages  = lt_messages ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
    mr_cut->save( ).
  ...
  METHOD u_agency_unknown.
    DATA(ls_travel) = _create_travel( VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_1  begin_date = '20190101'  end_date = '20190201' ) ).

    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_travel-travel_id  agency_id = mv_agency_id_unknown )
    ...
    cl_abap_unit_assert=>assert_equals( act = lx->mv_agency_id                   exp = mv_agency_id_unknown ).
  ...
  METHOD u_customer_unknown.
    ...
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_travel-travel_id  customer_id = mv_customer_id_unknown )
    ...
    cl_abap_unit_assert=>assert_equals( act = lx->mv_customer_id                 exp = mv_customer_id_unknown ).
  ...
  METHOD d_travel_id_initial.
    mr_cut->delete_travel( EXPORTING iv_travel_id = '0'
                           IMPORTING et_messages  = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy.
  ...
  METHOD u_travel_id_initial.
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( )
                                     is_travelx  = VALUE #( )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy ) ).
  ...
  METHOD act_set_status_to_booked.
    ...
    DATA(ls_travel_new) = _create_travel( VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_2  begin_date = '20190101'  end_date = '20190201' ) ).
    ...
    mr_cut->set_status_to_booked( EXPORTING iv_travel_id = ls_travel_new-travel_id
                                  IMPORTING et_messages  = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
    mr_cut->save( ).
    ...
    mr_cut->set_status_to_booked( EXPORTING iv_travel_id = '0'
                                  IMPORTING et_messages  = lt_messages ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy.
  ...
  METHOD c_dates_invalid.
    ...
    mr_cut->create_travel( EXPORTING is_travel   = VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_2  end_date = '20190101' )
                              IMPORTING es_travel   = DATA(ls_travel)
                                        et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( ls_travel-travel_id ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    ...
    mr_cut->create_travel( EXPORTING is_travel   = VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_2  begin_date = '20190101' )
    ...
    mr_cut->create_travel( EXPORTING is_travel   = VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_2  begin_date = '20190201'  end_date = '20190101' )
                              IMPORTING es_travel   = ls_travel
                                        et_messages = lt_messages ).
    cl_abap_unit_assert=>assert_initial( ls_travel-travel_id ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
  ...
  METHOD u_dates_invalid.
    ...
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_travel-travel_id )
    ...
    " Try to have begin date after end date
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_travel-travel_id  end_date = '20181201' )
                                     is_travelx  = VALUE #( travel_id = ls_travel-travel_id  end_date = abap_true )
                           IMPORTING et_messages = lt_messages ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy ) ).
  ...
  METHOD initialize.
    DATA(ls_travel_new) = _create_travel( VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_2  begin_date = '20190101'  end_date = '20190201'  description = 'My_Old_Text' ) ).

    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_travel_new-travel_id  description = 'My_New_Text' )
                                     is_travelx  = VALUE #( travel_id = ls_travel_new-travel_id  description = abap_true )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
    mr_cut->initialize( ).
  ...
  METHOD create_update_in_one_luw.
    mr_cut->create_travel( EXPORTING is_travel   = VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_2  begin_date = '20190101'  end_date = '20190201'  description = 'My_Old_Text' )
                           IMPORTING es_travel   = DATA(ls_travel)
                                     et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
    cl_abap_unit_assert=>assert_not_initial( ls_travel-travel_id ).

    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_travel-travel_id  agency_id = mv_agency_id_2  description = 'My_New_Text' )
    ...
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = lv_agency_id    exp = mv_agency_id_1 ).
  ...
  METHOD create_delete_in_one_luw.
    ...
    mr_cut->delete_travel( EXPORTING iv_travel_id = ls_travel-travel_id
                           IMPORTING et_messages  = lt_messages ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
  ...
  METHOD update_delete_in_one_luw.
    ...
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = lv_travel_id  agency_id = mv_agency_id_2  description = 'My_New_Text' )
                                     is_travelx  = VALUE #( travel_id = lv_travel_id  description = abap_true )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).

    mr_cut->delete_travel( EXPORTING iv_travel_id = lv_travel_id
                           IMPORTING et_messages  = lt_messages ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
  ...
  METHOD u_status_invalid.
    ...
    " No control data at all
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_travel-travel_id  status = lc_status_invalid )
...
CLASS ltc_booking DEFINITION FINAL FOR TESTING DURATION SHORT RISK LEVEL HARMLESS INHERITING FROM ltc_base.
  PRIVATE SECTION.
    ...
    "! Try to update a booking with invalid flight data -> ERROR
    METHODS u_flight_invalid         FOR TESTING RAISING cx_static_check.
...
CLASS ltc_booking IMPLEMENTATION.
  METHOD c_u_d_single.
    DATA(ls_booking) = _create_booking( iv_travel_id = mv_travel_id
                                        is_booking = VALUE #( booking_date  = mv_booking_date
                                                              booking_id    = mv_booking_id_unknown
                                                              customer_id   = mv_customer_id_1
                                                              carrier_id    = mv_carrier_id
                                                              connection_id = mv_connection_id
                                                              flight_date   = mv_flight_date_1 ) ).
    cl_abap_unit_assert=>assert_equals( act = ls_booking-customer_id    exp = mv_customer_id_1 ).
    cl_abap_unit_assert=>assert_equals( act = ls_booking-carrier_id     exp = mv_carrier_id ).
    cl_abap_unit_assert=>assert_equals( act = ls_booking-connection_id  exp = mv_connection_id ).
    ...
    DATA(lv_booking_date) = mv_booking_date + 15.
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
  ...
  METHOD cu_d_single.
    DATA(ls_booking) = _create_booking( iv_travel_id = mv_travel_id
                                        is_booking   = VALUE #( booking_id    = mv_booking_id_unknown
                                                                booking_date  = mv_booking_date
                                                                customer_id   = mv_customer_id_1
                                                                carrier_id    = mv_carrier_id
                                                                connection_id = mv_connection_id
                                                                flight_date   = mv_flight_date_1 )
  ...
  METHOD c_uu_d_single.
    ...
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
                                     is_travelx  = VALUE #( travel_id = mv_travel_id )
                                     it_booking  = VALUE #( ( booking_id = ls_booking-booking_id  flight_date = mv_flight_date_2 ) )
    ...
    SELECT SINGLE carrier_id, booking_date, flight_date FROM /dmo/booking WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO CORRESPONDING FIELDS OF @ls_booking_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = ls_booking_sel-carrier_id    exp = ls_booking-carrier_id ).
    cl_abap_unit_assert=>assert_equals( act = ls_booking_sel-booking_date  exp = lv_booking_date ).
    cl_abap_unit_assert=>assert_equals( act = ls_booking_sel-flight_date   exp = mv_flight_date_2 ).
  ...
  METHOD c_travel_id_unknown.
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id_unknown )
                                     is_travelx  = VALUE #( travel_id = mv_travel_id_unknown )
                                     it_booking  = VALUE #( ( booking_id = '10'  booking_date = '20180701'  customer_id = mv_customer_id_1  carrier_id = mv_carrier_id  connection_id = mv_connection_id ) )
  ...
  METHOD u_booking_id_unknown.
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
                                     is_travelx  = VALUE #( travel_id = mv_travel_id )
                                     it_booking  = VALUE #( ( booking_id = mv_booking_id_unknown  booking_date = '20180715'  ) )
                                     it_bookingx = VALUE #( ( booking_id = mv_booking_id_unknown  action_code = /dmo/if_flight_legacy=>action_code-update  booking_date = abap_true  ) )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy.
  ...
  METHOD d_booking_id_unknown.
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
                                     is_travelx  = VALUE #( travel_id = mv_travel_id )
                                     it_booking  = VALUE #( ( booking_id = mv_booking_id_unknown ) )
                                     it_bookingx = VALUE #( ( booking_id = mv_booking_id_unknown  action_code = /dmo/if_flight_legacy=>action_code-delete ) )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy.
  ...
  METHOD c_u_d_travel_w_bookings.
    ...
    DATA(ls_travel_1) = _create_travel( is_travel  = VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_1  begin_date = '20190101'  end_date = '20190201' )
                                        iv_save           = abap_false ).
    DATA(ls_travel_2) = _create_travel( is_travel  = VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_1  begin_date = '20190101'  end_date = '20190201' )
                                        iv_save           = abap_false ).
    cl_abap_unit_assert=>assert_not_initial( ls_travel_1-travel_id ).
    cl_abap_unit_assert=>assert_not_initial( ls_travel_2-travel_id ).
    ...
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_booking_1-travel_id )
                                     is_travelx  = VALUE #( travel_id = ls_booking_1-travel_id )
                                     it_booking  = VALUE #( ( booking_id = ls_booking_1-booking_id  customer_id = mv_customer_id_2 ) )
    ...
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = lv_customer_id  exp = mv_customer_id_2 ).
    ...
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = lv_customer_id  exp = mv_customer_id_1 ).
  ...
  METHOD c_customer_unknown.
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_booking  = VALUE #( ( booking_id    = mv_booking_id_unknown
                                                              booking_date  = mv_booking_date
                                                              customer_id   = mv_customer_id_unknown
                                                              carrier_id    = mv_carrier_id
                                                              connection_id = mv_connection_id
                                                              flight_date   = mv_flight_date_1 ) )
                                     it_bookingx = VALUE #( ( booking_id  = mv_booking_id_unknown
                                                              action_code = /dmo/if_flight_legacy=>action_code-create ) )
                           IMPORTING et_booking  = DATA(lt_booking)
                                     et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( lt_booking ).
  ...
  METHOD u_customer_unknown.
    ...
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
                                     is_travelx  = VALUE #( travel_id = mv_travel_id )
                                     it_booking  = VALUE #( ( booking_id = ls_booking-booking_id  customer_id = mv_customer_id_unknown ) )
  ...
  METHOD u_booking_date_initial.
    ...
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = lv_booking_date  exp = mv_booking_date ).
  ...
  METHOD c_flight_invalid.
    mr_cut->update_travel( EXPORTING is_travel    = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_booking   = VALUE #( ( booking_id    = mv_booking_id_unknown
                                                               booking_date  = mv_booking_date
                                                               customer_id   = mv_customer_id_1
                                                               carrier_id    = mv_carrier_id_unknown
                                                               connection_id = mv_connection_id
                                                               flight_date   = mv_flight_date_1 ) )
                                      it_bookingx = VALUE #( ( booking_id    = mv_booking_id_unknown
                                                               action_code   = /dmo/if_flight_legacy=>action_code-create ) )
    ...
    cl_abap_unit_assert=>assert_equals( act = lx->mv_carrier_id                  exp = mv_carrier_id_unknown ).
    cl_abap_unit_assert=>assert_equals( act = lx->mv_connection_id               exp = mv_connection_id ).
    cl_abap_unit_assert=>assert_equals( act = lx->mv_flight_date                 exp = mv_flight_date_1 ).
  ENDMETHOD.


  METHOD u_flight_invalid.
    DATA(ls_booking) = _create_booking( iv_travel_id = mv_travel_id
                                        is_booking   = VALUE #( booking_id    = mv_booking_id_unknown
                                                                booking_date  = mv_booking_date
                                                                customer_id   = mv_customer_id_1
    ...
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
                                     is_travelx  = VALUE #( travel_id = mv_travel_id )
                                     it_booking  = VALUE #( ( booking_id = ls_booking-booking_id  carrier_id = mv_carrier_id_unknown ) )
                                     it_bookingx = VALUE #( ( booking_id = ls_booking-booking_id  action_code = /dmo/if_flight_legacy=>action_code-update  carrier_id = abap_true ) )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy.
    ...
    SELECT SINGLE carrier_id FROM /dmo/booking WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO @DATA(lv_carrier_id).
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = lv_carrier_id  exp = mv_carrier_id ).
...
CLASS ltc_booking_supplement DEFINITION FINAL FOR TESTING DURATION SHORT RISK LEVEL HARMLESS INHERITING FROM ltc_base.
  PRIVATE SECTION.
    ...
    "! Try to update a Booking Supplement with an unknown supplement ID -> ERROR
    METHODS u_supplement_unknown         FOR TESTING RAISING cx_static_check.
...
CLASS ltc_booking_supplement IMPLEMENTATION.
  METHOD c_u_d_multiple.
    ...
    " 1. LUW:
    " Create a travel
    DATA(ls_travel) = _create_travel( is_travel = VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_1  begin_date = '20190101'  end_date = '20190201' )
                                      iv_save   = abap_false ).
    cl_abap_unit_assert=>assert_not_initial( ls_travel-travel_id ).
    ...
    _create_booking_suppl( iv_travel_id          = ls_booking_1-travel_id
                           is_booking_supplement = VALUE #( booking_id            = ls_booking_1-booking_id
                                                            ...
                                                            supplement_id         = mv_supplement_id_1
                                                            price                 = '10.0'
                                                            currency_code         = 'EUR' )
                           iv_save               = abap_false ).
    ...
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = ls_book_suppl_sel-price          exp = mv_supplement_price_1 ).
    cl_abap_unit_assert=>assert_equals( act = ls_book_suppl_sel-currency_code  exp = mv_supplement_currency_code_1 ).

    " 2. Update the first booking supplement of the second booking
    mr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = ls_book_suppl_2_1-travel_id )
                                     ...
                                     it_booking_supplement  = VALUE #( ( booking_id            = ls_book_suppl_2_1-booking_id
                                                                         ...
                                                                         price                 = '21.0'
                                                                         currency_code         = 'XXX' "To be ignored
                                                                         ) )
    ...
    _delete_existing_booking_suppl( iv_travel_id = ls_book_suppl_2_2-travel_id  iv_booking_id = ls_book_suppl_2_2-booking_id  iv_booking_supplement_id = ls_book_suppl_2_2-booking_supplement_id ).
  ...
  METHOD u_booking_suppl_id_unknown.
    mr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = mv_travel_id )
                                     is_travelx             = VALUE #( travel_id = mv_travel_id )
                                     it_booking_supplement  = VALUE #( ( booking_id = mv_booking_id  booking_supplement_id = mv_booking_suppl_id_unknown  price = '11.0' ) )
                                     it_booking_supplementx = VALUE #( ( booking_id = mv_booking_id  booking_supplement_id = mv_booking_suppl_id_unknown  action_code = /dmo/if_flight_legacy=>action_code-update  price = abap_true ) )
                           IMPORTING et_messages            = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy.
    ...
    cl_abap_unit_assert=>assert_equals( act = lx->mv_booking_supplement_id       exp = mv_booking_suppl_id_unknown ).
  ENDMETHOD.
  ...
  METHOD d_booking_suppl_id_unknown.
    mr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = mv_travel_id )
                                     is_travelx             = VALUE #( travel_id = mv_travel_id )
                                     it_booking_supplement  = VALUE #( ( booking_id = mv_booking_id  booking_supplement_id = mv_booking_suppl_id_unknown ) )
                                     it_booking_supplementx = VALUE #( ( booking_id = mv_booking_id  booking_supplement_id = mv_booking_suppl_id_unknown
                                                                         action_code = /dmo/if_flight_legacy=>action_code-delete ) )
                           IMPORTING et_messages            = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy ) ).
  ...
  METHOD d_travel_c_bsuppl_1_luw.
    ...
    mr_cut->delete_travel( EXPORTING iv_travel_id = ls_travel-travel_id
                           IMPORTING et_messages  = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
    mr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = ls_booking-travel_id )
  ...
  METHOD d_booking_c_bsuppl_1_luw.
    ...
    " 2. LUW: Delete Booking, try to create Booking Supplement
    mr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_booking-travel_id )
                                     is_travelx  = VALUE #( travel_id = ls_booking-travel_id )
                                     it_booking  = VALUE #( ( booking_id = ls_booking-booking_id ) )
                                     it_bookingx = VALUE #( ( booking_id = ls_booking-booking_id  action_code = /dmo/if_flight_legacy=>action_code-delete ) )
  ...
  METHOD du_single.
    ...
    " Create and Persist: Travel - Booking - Booking Supplement
    DATA(ls_travel) = _create_travel( is_travel = VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_1  begin_date = '20190101'  end_date = '20190201' )
                                      iv_save   = abap_false ).
    DATA(ls_booking) = _create_booking( iv_travel_id = ls_travel-travel_id
                                        is_booking   = VALUE #( booking_id    = '10'
    ...
    mr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = ls_booking_supplement-travel_id )
  ...
  METHOD c_supplement_unknown.
    ...
    " Create and Persist: Travel - Booking
    DATA(ls_travel) = _create_travel( is_travel = VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_1  begin_date = '20190101'  end_date = '20190201' )
                                      iv_save   = abap_false ).
    DATA(ls_booking) = _create_booking( iv_travel_id = ls_travel-travel_id
                                        is_booking   = VALUE #( booking_id    = '10'
    ...
    mr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = ls_booking-travel_id )
                                     ...
                                     it_booking_supplement  = VALUE #( ( booking_id            = ls_booking-booking_id
                                                                         booking_supplement_id = '1'
                                                                         supplement_id         = mv_supplement_id_unknown
                                                                         price                 = '10.0'
                                                                         currency_code         = 'EUR' )
    ...
    cl_abap_unit_assert=>assert_equals( act = lx->mv_supplement_id               exp = mv_supplement_id_unknown ).

    mr_cut->save( ).

    " Check that no Booking Supplement has been created
    CLEAR lv_db_exists.
    SELECT SINGLE FROM /dmo/book_suppl FIELDS @abap_true WHERE travel_id  = @ls_booking-travel_id
  ...
  METHOD u_supplement_unknown.
    ...
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = lv_supplement_id  exp = mv_supplement_id_1 ).
    cl_abap_unit_assert=>assert_equals( act = lv_price          exp = mv_supplement_price_1 ).
  ...
  METHOD c_booking_suppl_id_initial.
    ...
    mr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = ls_booking-travel_id )
                                     ...
                                     it_booking_supplement  = VALUE #( ( booking_id            = ls_booking-booking_id
                                                                         ...
                                                                         supplement_id         = mv_supplement_id_1
                                                                         price                 = mv_supplement_price_1
                                                                         currency_code         = mv_supplement_currency_code_1 ) )
                                     it_booking_supplementx = VALUE #( ( booking_id            = ls_booking-booking_id
                                                                         booking_supplement_id = '0'
                                                                         action_code           = /dmo/if_flight_legacy=>action_code-create ) )
                           IMPORTING et_booking_supplement  = DATA(lt_booking_supplement)
  ...
  METHOD c_booking_suppl_id_exists.
    ...
    _create_booking_suppl( iv_travel_id          = ls_booking-travel_id
                           is_booking_supplement = VALUE #( booking_id            = ls_booking-booking_id
                                                            booking_supplement_id = '20'
                                                            supplement_id         = mv_supplement_id_1
                                                            price                 = mv_supplement_price_1
  ...
  METHOD c_deep_insert.
    ...
    mr_cut->create_travel(
      EXPORTING
        is_travel             = VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_1  begin_date = '20190101'  end_date = '20190201'  description = 'My_Deep_Insert' )
        it_booking            = VALUE #( ( booking_id    = lc_booking_id_1
                                         ...
                                         ( booking_id    = lc_booking_id_2
                                           booking_date  = mv_booking_date
                                           customer_id   = mv_customer_id_2
                                           carrier_id    = mv_carrier_id
                                           connection_id = mv_connection_id
                                           flight_date   = mv_flight_date_2 ) )
    ...
    " Check travel
    DATA ls_travel_sel TYPE /dmo/travel.
    SELECT SINGLE agency_id, customer_id, description FROM /dmo/travel WHERE travel_id = @ls_travel-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-agency_id    exp = mv_agency_id_1 ).
    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-customer_id  exp = mv_customer_id_1 ).
    ...
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = ls_booking_sel-customer_id  exp = mv_customer_id_2 ).
    cl_abap_unit_assert=>assert_equals( act = ls_booking_sel-flight_date  exp = mv_flight_date_2 ).
  ...
  METHOD c_deep_insert_suppl_unknown.
    ...
    mr_cut->create_travel(
      EXPORTING
        is_travel             = VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_1  begin_date = '20190101'  end_date = '20190201'  description = 'My_Deep_Insert_Fail' )
        it_booking            = VALUE #( ( booking_id    = lc_booking_id_1
                                           booking_date  = mv_booking_date
                                           customer_id   = mv_customer_id_1
                                           carrier_id    = mv_carrier_id
         ...
         it_booking_supplement = VALUE #( ( booking_id            = lc_booking_id_2
                                            booking_supplement_id = lc_booking_supplement_id_2_1
                                            supplement_id         = mv_supplement_id_unknown " <<< Unknown Supplement ID
                                            price                 = '20.0'
                                            currency_code         = 'EUR' ) )
      IMPORTING
  ...
  METHOD u_single_travel_multiple.
    ...
    mr_cut->create_travel(
      EXPORTING
        is_travel             = VALUE #( agency_id = mv_agency_id_1  customer_id = mv_customer_id_1  begin_date = '20190101'  end_date = '20190201'  description = 'My_Deep_Insert_1' )
        it_booking            = VALUE #( ( booking_id    = lc_booking_id_1
                                           booking_date  = mv_booking_date
                                           customer_id   = mv_customer_id_1
                                           carrier_id    = mv_carrier_id
    ...
    mr_cut->update_travel(
      EXPORTING
        is_travel              = VALUE #( travel_id = ls_travel-travel_id  description = 'My_Deep_Insert_2' )
        is_travelx             = VALUE #( travel_id = ls_travel-travel_id  description = abap_true )
        it_booking             = VALUE #( ( booking_id = lc_booking_id_1  customer_id = mv_customer_id_2 )
                                          ( booking_id    = lc_booking_id_2
                                            booking_date  = mv_booking_date
                                            customer_id   = mv_customer_id_1
                                            carrier_id    = mv_carrier_id
        ...
        it_booking_supplement  = VALUE #( ( booking_id            = lc_booking_id_1
                                          ...
                                          ( booking_id            = lc_booking_id_1
                                            ...
                                            supplement_id         = mv_supplement_id_2
                                            price                 = '10.0'
                                            currency_code         = 'EUR' )
                                          ( booking_id            = lc_booking_id_2
                                            booking_supplement_id = lc_booking_supplement_id_2_1
    ...
    " Check Travel
    DATA ls_travel_sel TYPE /dmo/travel.
    SELECT SINGLE description FROM /dmo/travel WHERE travel_id = @ls_travel-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    ...
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = ls_booking_sel-booking_date  exp = mv_booking_date ).
    cl_abap_unit_assert=>assert_equals( act = ls_booking_sel-customer_id   exp = mv_customer_id_2 ).
    ...
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = ls_booking_sel-customer_id  exp = mv_customer_id_1 ).
    cl_abap_unit_assert=>assert_equals( act = ls_booking_sel-flight_date  exp = mv_flight_date_2 ).
    ...
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = ls_booking_supplement_sel-supplement_id  exp = mv_supplement_id_1 ).
    ...
    SELECT SINGLE supplement_id FROM /dmo/book_suppl WHERE travel_id             = @ls_travel-travel_id
                                                     ...
                                                     INTO CORRESPONDING FIELDS OF @ls_booking_supplement_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = ls_booking_supplement_sel-supplement_id  exp = mv_supplement_id_2 ).
  ...
  METHOD read_db.
    ...
    mr_cut->get_travel( EXPORTING iv_travel_id          = ls_travel-travel_id
    ...
    cl_abap_unit_assert=>assert_equals( act = lt_booking_supplement_read[ travel_id = ls_travel-travel_id  booking_id = '21'  booking_supplement_id = '20' ]-supplement_id  exp = mv_supplement_id_2 ).
  ...
  METHOD read_travel_id_initial.
    mr_cut->get_travel( EXPORTING iv_travel_id          = '0'
                        IMPORTING es_travel             = DATA(ls_travel_read)
                                  et_booking            = DATA(lt_booking_read)
                                  et_booking_supplement = DATA(lt_booking_supplement_read)
                                  et_messages           = DATA(lt_messages) ).
